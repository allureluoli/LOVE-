...
jrrp = on_command('今日抽签', aliases={'运势', '今日运势', 'jrrp'}, priority=50)
...
async def handle_func(bot: Bot, event: GroupMessageEvent):
    ...
    listFiles = os.listdir(os.getcwd() + "/love/data/Emoticon/mywife/")
    path = Path(os.getcwd() + f"/love/data/Emoticon/mywife/{listFiles[x]}").parent / f"{listFiles[x]}"
...
wife = on_fullmatch('。。', priority=50)


@wife.handle()
async def handle_func(bot: Bot, event: Event):
    await wife.send("两个泡泡")


abuse = on_fullmatch('骂我', rule=to_me(), priority=50)
